...
example report, upload the graph data file at: http://edwards.sdsu.edu/prinseq
...
seq_num, trim_left, trim_right, trim_qual_left, trim_qual_right, trim_tail_left, trim_tail_right, trim_ns_left, trim_ns_right, trim_to_len, min_len, max_len, range_len, min_qual_score, max_qual_score, min_qual_mean, max_qual_mean, min_gc, max_gc, range_gc, ns_max_p, ns_max_n, noniupac, lc_method, derep, seq_id, seq_case, dna_rna, out_format
...
my ($file1,$command,@dataread,$aa,%filtercount);
...
my ($fhgood,$fhgood2,$fhgood3,$fhbad,$fhbad2,$fhbad3,$fhmappings);
my ($filenamegood,$filenamegood2,$filenamegood3,$filenamebad,$filenamebad2,$filenamebad3,$filenamemappings);
...
my $goodcount = 0;
my $badcount = 0;
my ($tsvfile,$seqid,$header,$seq,$qual,$count,$numbases,$length,$seqgd);
$count = 0;
$qual = '';
$seq = '';
#stats data
my (%stats,%kmers,%odds,%counts,%graphdata,$md5,$md5r,%md5s,%md5sg,$ucseq,$maxlength);
$maxlength = 0;
#parse data
my $seqcount = 0;
my $seqbases = 0;
my $badbases = 0;
my (@seqs,@seqsP,@printtmp); #seqs for stats and graph data, seqsP for preprocessing sequences - all used for duplicate checking/removal
        ...
        if($count == 0 && /^\@(\S+)\s*(.*)$/o) {
            ...
            if($length) {
                ...
                if($exists_stats) {
                    ...
                    if($exists_graphdata) {
                        $ucseq = uc($seq);
                        &getSeqStats($ucseq,$length);
                        if($qual) {
                            &getQualStats($qual,$length);
                        }
...
print STDERR "\tdone\n" if(exists $params{verbose} && !exists $params{stats});
if((exists $params{verbose} && !exists $params{stats}) || !$webnoprocess) {
...
if(exists $params{graph_data}) {
	...
	my %prob;
	#calculate frequency of 5-mers
        my $kmersum = &getTagFrequency($graphdata{kmers},$seqcount);
        #check for frequency of MID tags
        my $midsum = 0;
        my $midcount = 0;
        my @midseqs;
	...
	foreach my $i (keys %{$graphdata{dinucodds}}) {
	    $graphdata{dinucodds}->{$i} = sprintf("%.9f",$graphdata{dinucodds}->{$i}/$numseqs);
	}
    }
    #check for n-plicates
    ...
    my $str = '';
    $str .= '{"numseqs":'.$numseqs.',"numbases":'.$numbases.',"maxlength":'.$maxlength.',"binval":'.$binval.',"exactonly":'.$exactonly.',"tagmidnum":'.($graphdata{tagmidnum}||0).',"scale":'.$scale.',"filename1":"'.(exists $params{filename1} ? $params{filename1} : &convertStringToInt(&getFileName($file1))).'","format1":"'.(exists $params{fasta} ? 'fasta' : 'fastq').'"'.(exists $params{qual} ? ',"filename2":"'.(exists $params{filename2} ? $params{filename2} : &convertStringToInt(&getFileName($params{qual}))).'","format2":"qual"' : '');
    foreach my $s qw(counts stats quals qualsbin complvals dubscounts dubslength) {
    ...
    foreach my $s qw(qualsmean tagprob compldust complentropy dinucodds) {
        next unless exists($graphdata{$s});
        $str .= ',"'.$s.'":{';
    ...
    open(FH, ">", $params{graph_data}) or die "ERROR: Can't open file ".$params{graph_data}.": $! \n";
    flock(FH, LOCK_EX) or die "ERROR: Cannot lock file ".$params{graph_data}.": $! \n";
    print FH "#Graph data\n";
    print FH "#[prinseq-".$WHAT."-$VERSION] [$time] Command: \"perl prinseq-".$WHAT.".pl".$command."\"\n" unless(exists $params{web});
    print FH $str;
    flock(FH, LOCK_UN) or die "ERROR: cannot unlock ".$params{graph_data}.": $! \n";
    close(FH);
