...
from .core.misc import ReadIdStore
...
def parser_index_fasta_samtools(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None)))
    util_cmd.attach_main(parser, main_index_fasta_samtools)
    return parser


def main_index_fasta_samtools(args):
    '''Index a reference genome for Samtools.'''
    samtools.SamtoolsTool().faidx(args.inFasta, overwrite=True)
    return 0
...
def parser_index_fasta_picard(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        '--JVMmemory',
        default=picard.CreateSequenceDictionaryTool.jvmMemDefault,
...
def parser_mkdup_picard(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        '--JVMmemory',
        default=picard.MarkDuplicatesTool.jvmMemDefault,
        help='JVM virtual memory size (default: %(default)s)'
    )
    parser.add_argument(
        '--picardOptions',
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_mkdup_picard)
    return parser
...
def main_mkdup_picard(args):
    ...
    picard.MarkDuplicatesTool().execute(
...
def parser_revert_sam_common(parser=argparse.ArgumentParser()):
    ...
    try:
        parser.add_argument(
            '--JVMmemory',
            default=picard.RevertSamTool.jvmMemDefault,
            help='JVM virtual memory size (default: %(default)s)'
        )
...
def parser_revert_bam_picard(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_revert_bam_picard, split_args=True)
    return parser
...
def main_revert_bam_picard(inBam, outBam, clear_tags=False, tags_to_clear=None, picardOptions=None, do_not_sanitize=False, JVMmemory=None):
    ...
    picard.RevertSamTool().execute(inBam, outBam, picardOptions=picardOptions, JVMmemory=JVMmemory)
    return 0
...
def revert_bam_if_aligned(inBam, revert_bam=None, clear_tags=True, tags_to_clear=None, picardOptions=None, JVMmemory="4g", sanitize=False):
    ...
    revertBamOut = revert_bam if revert_bam else util_file.mkstempfname('.bam')
    picardOptions = picardOptions or []
    tags_to_clear = tags_to_clear or []
...
def parser_picard(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_picard)
    return parser


def main_picard(args):
    '''Generic Picard runner.'''
    picard.PicardTools().execute(args.command, picardOptions=args.picardOptions, JVMmemory=args.JVMmemory)
    return 0
...
def parser_sort_bam(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        ...
        choices=picard.SortSamTool.valid_sort_orders,
        default=picard.SortSamTool.default_sort_order
    )
    parser.add_argument(
        "--index",
    ...
    parser.add_argument(
        '--JVMmemory',
        default=picard.SortSamTool.jvmMemDefault,
        help='JVM virtual memory size (default: %(default)s)'
    )
    parser.add_argument(
        '--picardOptions',
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_sort_bam)
    return parser
...
def parser_downsample_bams(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, main_downsample_bams, split_args=True)
    return parser
...
def main_downsample_bams(in_bams, out_path, specified_read_count=None, deduplicate_before=False, deduplicate_after=False, picardOptions=None, threads=None, JVMmemory=None):
    ...
    def downsample_bams(data_pairs, downsample_target, threads=None, JVMmemory=None):
        ...
        JVMmemory = JVMmemory if JVMmemory else picard.DownsampleSamTool.jvmMemDefault
        jvm_worker_memory_mb = str(int(util_misc.convert_size_str(JVMmemory,"m")[:-1])//workers)+"m"
    ...
    if deduplicate_before:
        with util_file.tempfnames(suffixes=[ '{}.dedup.bam'.format(os.path.splitext(os.path.basename(x))[0]) for x in in_bams]) as deduped_bams:
        ...
        if deduplicate_after:
            ...
            with util_file.tempfnames(suffixes=[ '{}.downsampled-{}.bam'.format(os.path.splitext(os.path.basename(x))[0], downsample_target) for x in in_bams]) as downsampled_tmp_bams:
...
def parser_merge_bams(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        '--JVMmemory',
        default=picard.MergeSamFilesTool.jvmMemDefault,
        help='JVM virtual memory size (default: %(default)s)'
    )
    parser.add_argument(
        '--picardOptions',
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_merge_bams)
    return parser
...
def main_merge_bams(args):
    ...
    picard.MergeSamFilesTool().execute(args.inBams, args.outBam, picardOptions=opts, JVMmemory=args.JVMmemory)
    return 0
...
def parser_filter_bam(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_filter_bam)
    return parser


def main_filter_bam(args):
    '''Filter BAM file by read name'''
    with util_file.tmp_dir(suffix='_filter_bam') as tmpdir:
...
def fastq_to_bam(
    ...
    if header:
        samtools.SamtoolsTool().reheader(fastqToSamOut, header, outBam)
...
def parser_fastq_to_bam(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, fastq_to_bam, split_args=True)
    return parser
...
def join_paired_fastq(
    ...
    if output == '-':
        output = sys.stdout
    SeqIO.write(util_file.join_paired_fastq(inFastqs, output_format=outFormat), output, outFormat)
    return 0
...
def parser_join_paired_fastq(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, join_paired_fastq, split_args=True)
    return parser
...
def parser_split_bam(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, split_bam, split_args=True)
    return parser
...
def parser_reheader_bam(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('outBam', help='Output reads, BAM format.')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_reheader_bam)
    return parser
...
def main_reheader_bam(args):
    ...
    '''
    # read mapping file
    mapper = dict((a + ':' + b, a + ':' + c) for a, b, c in util_file.read_tabfile(args.rgMap))
    # read and convert bam header
    header_file = mkstempfname('.sam')
    with open(header_file, 'wt') as outf:
        for row in samtools.SamtoolsTool().getHeader(args.inBam):
            if row[0] == '@RG':
                row = [mapper.get(x, x) for x in row]
            outf.write('\t'.join(row) + '\n')
    # write new bam with new header
    samtools.SamtoolsTool().reheader(args.inBam, header_file, args.outBam)
...
def parser_reheader_bams(parser=argparse.ArgumentParser()):
    parser.add_argument('rgMap', help='Tabular file containing three columns: field, old, new.')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_reheader_bams)
    return parser
...
def main_reheader_bams(args):
    ...
    '''
    # read mapping file
    mapper = dict((a + ':' + b, a + ':' + c) for a, b, c in util_file.read_tabfile(args.rgMap) if a != 'FN')
    files = list((b, c) for a, b, c in util_file.read_tabfile(args.rgMap) if a == 'FN')
    header_file = mkstempfname('.sam')
    # read and convert bam headers
    for inBam, outBam in files:
        if os.path.isfile(inBam):
            with open(header_file, 'wt') as outf:
                for row in samtools.SamtoolsTool().getHeader(inBam):
                    if row[0] == '@RG':
                        row = [mapper.get(x, x) for x in row]
                    outf.write('\t'.join(row) + '\n')
            # write new bam with new header
            samtools.SamtoolsTool().reheader(inBam, header_file, outBam)
    os.unlink(header_file)
    return 0
...
def parser_rmdup_bbnorm_bam(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, rmdup_bbnorm_bam, split_args=True)
    return parser
...
def parser_rmdup_prinseq_fastq(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_rmdup_prinseq_fastq)
    return parser
...
def filter_bam_mapped_only(inBam, outBam):
    ...
    '''
    samtools.SamtoolsTool().view(['-b', '-q', '1', '-F', '1028'], inBam, outBam)
    picard.BuildBamIndexTool().execute(outBam)
    return 0
...
def parser_filter_bam_mapped_only(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, filter_bam_mapped_only, split_args=True)
    return parser
...
def parser_novoalign(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_novoalign)
    return parser


def main_novoalign(args):
    '''Align reads with Novoalign. Sort and index BAM output.'''
    novoalign.NovoalignTool(license_path=args.novoalign_license_path).execute(
        args.inBam,
        args.refFasta,
        args.outBam,
...
def parser_novoindex(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None)))
    util_cmd.attach_main(parser, main_novoindex)
    return parser

def main_novoindex(args):
    novoalign.NovoalignTool(license_path=args.novoalign_license_path).index_fasta(args.refFasta)
    return 0
...
def align_and_fix(
    ...
    if aligner=="novoalign":
        ...
        novoalign.NovoalignTool(license_path=novoalign_license_path).index_fasta(refFastaCopy)
    ...
    if outBamAll:
        ...
        if dup_marker == 'sambamba':
            ...
            picard.BuildBamIndexTool().execute(outBamAll, JVMmemory=JVMmemory)
    if outBamFiltered:
...
def parser_filter_bam_to_proper_primary_mapped_reads(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, filter_bam_to_proper_primary_mapped_reads, split_args=True)
    return parser
...
def parser_minimap2_idxstats(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, minimap2_idxstats, split_args=True)
    return parser
...
def bwamem_idxstats(inBam, refFasta, outBam=None, outStats=None,
    ...
    bam_aligned = util_file.mkstempfname('.aligned.bam')
    if outBam is None:
...
def parser_bwamem_idxstats(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, bwamem_idxstats, split_args=True)
    return parser
...
def parser_extract_tarball(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('--pipe_hint',
        ...
        default=None)
    util_cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, main_extract_tarball, split_args=True)
    return parser
def main_extract_tarball(*args, **kwargs):
    ...
    '''
    print(util_file.extract_tarball(*args, **kwargs))
...
def fasta_read_names(in_fasta, out_read_names):
    ...
    with util_file.open_or_gzopen(in_fasta) as in_fasta_f, open(out_read_names, 'wt') as out_read_names_f:
        last_read_name = None
...
def read_names(in_reads, out_read_names, threads=None):
    ...
    _in_reads = in_reads
    with util_file.tmp_dir(suffix='_read_names.txt') as t_dir:
...
def parser_read_names(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, read_names, split_args=True)
    return parser
...
def trim_rmdup_subsamp_reads(inBam, clipDb, outBam, n_reads=100000, trim_opts=None, threads=None):
    ...
    with util_file.tempfnames(suffixes=['.in.1.fastq', '.in.2.fastq']) as infq, \
         util_file.tempfnames(suffixes=['.trim.1.fastq', '.trim.2.fastq']) as trimfq, \
         util_file.tempfnames(suffixes=['.trim.unpaired.1.fastq', '.trim.unpaired.2.fastq']) as trimfq_unpaired, \
         util_file.tempfnames(suffixes=['.rmdup.1.fastq', '.rmdup.2.fastq']) as rmdupfq, \
         util_file.tempfnames(suffixes=['.rmdup.unpaired.1.fastq', '.rmdup.unpaired.2.fastq']) as rmdupfq_unpaired_from_paired_rmdup, \
         util_file.tempfname(suffix='.header.sam') as tmp_header, \
         util_file.tempfname(suffix='.paired.bam') as tmp_bam_paired:

        # BAM -> fastq
        picard.SamToFastqTool().execute(inBam, infq[0], infq[1])
        n_input = util_file.count_fastq_reads(infq[0])
        ...
        if n_input == 0:
            ...
            trimmomatic.TrimmomaticTool().execute(
                infq[0],
                infq[1],
                trimfq[0],
                trimfq[1],
        ...
        n_trim = max(map(util_file.count_fastq_reads, trimfq))    # count is pairs
        ...
        if all(os.path.getsize(x) > 0 for x in rmdupfq):
            picard.FastqToSamTool().execute(rmdupfq[0], rmdupfq[1], 'Dummy', tmp_bam_paired)
        else:
            opts = ['INPUT=' + tmp_header, 'OUTPUT=' + tmp_bam_paired, 'VERBOSITY=ERROR']
            picard.PicardTools().execute('SamFormatConverter', opts, JVMmemory='50m')
        ...
        if n_rmdup_paired * 2 < n_reads:
            ...
            with util_file.tempfname(suffix='.unpaired.fastq') as unpaired_concat, \
                 util_file.tempfname(suffix='.unpaired.rmdup.fastq') as unpaired_concat_rmdup:
                ...
                if n_rmdup_unpaired == 0:
                    ...
                    with util_file.tempfname(suffix='.unpaired.bam') as tmp_bam_unpaired, \
                         ...
                         util_file.tempfname(suffix='.merged.bam') as tmp_bam_merged:

                        # take pooled unpaired reads and convert to bam
                        picard.FastqToSamTool().execute(unpaired_concat_rmdup, None, 'Dummy', tmp_bam_unpaired)
...
def parser_trim_rmdup_subsamp(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, trim_rmdup_subsamp_reads, split_args=True)
    return parser
