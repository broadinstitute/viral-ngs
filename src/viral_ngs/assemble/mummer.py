...
class MummerTool(viral_ngs.core.Tool):
    ...
    def executable_path(self):
        exec_path = viral_ngs.core.Tool.executable_path(self)
    ...
    def trim_contigs(self, refFasta, contigsFasta, outFasta,
        ...
        if aligner=='nucmer':
            ...
            raise NameError()
        delta_1 = viral_ngs.core.file.mkstempfname('.delta')
        delta_2 = viral_ngs.core.file.mkstempfname('.delta')
        tiling = viral_ngs.core.file.mkstempfname('.tiling')
    ...
    def scaffold_contigs_custom(self, refFasta, contigsFasta, outFasta,
        ...
        with viral_ngs.core.file.open_or_gzopen(tiling, 'rU') as inf:
            for line in inf:
                row = line.rstrip('\n\r').split('\t')
                c = row[11]
        ...
        alnReaders = {}
        aln_file = viral_ngs.core.file.mkstempfname('.aligns')
        ...
        with open(outFasta, 'wt') as outf:
            for c in [seqObj.id for seqObj in Bio.SeqIO.parse(refFasta, 'fasta')]:
                ...
                def frac_unambig(seqs):
                    ...
                    viral_ngs.core.misc.chk(n_diff_lens(alt_seqs_f) == 1, 'ambig_max_lens>1 not currently supported')
                    n_tot = len(seqs[0])
                ...
                for line in viral_ngs.core.file.fastaMaker([(str(c)+"_contigs_ordered_and_oriented", ''.join(seq))]):
                    outf.write(line)
        ...
        if outAlternateContigs and alternate_contigs:
            ...
            with open(outAlternateContigs, 'wt') as outf:
                for c, left, right, seqs in alternate_contigs:
                    for line in viral_ngs.core.file.fastaMaker([
    ...
    def align_one_to_one(self, refFasta, otherFasta, outFasta):
        ...
        viral_ngs.core.file.makeFastaFile(seqs, outFasta)
...
def contig_chooser(alt_seqs, ref_len, coords_debug=""):
    ...
    if not alt_seqs:
        ...
        ranks = list(sorted(viral_ngs.core.misc.histogram(alt_seqs).items(),
        ...
        if len(ranks)==1:
            ...
            len_ranks = list(sorted(viral_ngs.core.misc.histogram(
