...
def merge_tarballs(out_tarball, in_tarballs, threads=None, extract_to_disk_path=None, pipe_hint_in=None, pipe_hint_out=None):
    ...
    '''
    util_file.repack_tarballs(out_tarball, in_tarballs, threads=threads, extract_to_disk_path=extract_to_disk_path, pipe_hint_in=pipe_hint_in, pipe_hint_out=pipe_hint_out)
    return 0
def parser_merge_tarballs(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, merge_tarballs, split_args=True)
    return parser
...
def parser_rename_fasta_sequences(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('tmp_dir', None), ('loglevel', None), ('version', None)))
    util_cmd.attach_main(parser, main_rename_fasta_sequences)
    return parser
def main_rename_fasta_sequences(args):
    ...
    '''
    n_seqs = util_file.fasta_length(args.in_fasta)
...
def parser_tsv_derived_cols(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, tsv_derived_cols, split_args=True)
    return parser
...
def parser_tsv_join(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None)))
    util_cmd.attach_main(parser, tsv_join, split_args=True)
    return parser
def tsv_join(in_tsvs, out_tsv, join_id=None):
    ...
    header = list(util_misc.unique(header))
    ...
    out_ids = list(util_misc.unique(out_ids))
