...
def get_assembly_stats(sample,
    ...
    if os.path.isdir(raw_reads_dir):
        out['reads_raw'] = sum(samtools.count(bam)
            ...
            # and leading/trailing dots are stripped from sample names in util_file.string_to_file_name()
    ...
    if os.path.isfile(assembly_fname):
        with open(assembly_fname, 'rt') as inf:
            counts = [(len(s), util_misc.unambig_count(s.seq)) for s in Bio.SeqIO.parse(inf, 'fasta') if len(s) > 0]
...
def parser_assembly_stats(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, assembly_stats, split_args=True)
    return parser
...
def parser_coverage_only(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('--cov_thresholds',
                        ...
                        help='Genome coverage thresholds to report on. (default: %(default)s)')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, coverage_only, split_args=True)
    return parser
...
def consolidate_fastqc(inDirs, outFile):
    ...
    with util_file.open_or_gzopen(outFile, 'wt') as outf:
        header = ['Sample']
        out_n = 0
        for sdir in inDirs:
            out = {}
...
def parser_consolidate_fastqc(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('outFile', help='Output report file.')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, consolidate_fastqc, split_args=True)
    return parser
...
def get_bam_info(bamstats_dir):
    ...
    for fn in glob.glob(os.path.join(bamstats_dir, "*.txt")):
        with util_file.open_or_gzopen(fn, 'rt') as inf:
            bam = {}
            for line in inf:
                k, v = line.rstrip('\n').split('\t')
                bam[k] = v
...
def parser_consolidate_spike_count(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, consolidate_spike_count, split_args=True)
    return parser
...
def parser_aggregate_spike_count(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('out_file', metavar="outFile", help='Output report file.')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, aggregate_spike_count, split_args=True)
    return parser
...
def parser_aggregate_alignment_counts(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('out_file', metavar="outFile", help='Output report file.')
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, aggregate_alignment_counts, split_args=True)
    return parser
...
def plot_coverage(
    ...
    if out_summary is None:
        coverage_tsv_file = util_file.mkstempfname('.summary.tsv')
    else:
        coverage_tsv_file = out_summary

    bam_dupe_processed = util_file.mkstempfname('.dupe_processed.bam')
...
def parser_plot_coverage(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, plot_coverage, split_args=True)
    return parser
...
def align_and_plot_coverage(
    ...
    if excludeDuplicates:
        ...
        dupe_removal_out_metrics = util_file.mkstempfname('.metrics')
...
def parser_align_and_plot_coverage(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, align_and_plot_coverage, split_args=True)
    return parser
...
def parser_fastqc(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, fastqc.FastQC().execute, split_args=True)
    return parser
