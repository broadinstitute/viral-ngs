...
# Note: avoid circular import by importing submodules directly, not the util package
from . import version as util_version
from . import misc as util_misc
from . import file as util_file

__author__ = "dpark@broadinstitute.org"
__version__ = util_version.get_version()
...
def common_args(parser, arglist=(('tmp_dir', None), ('loglevel', None))):
    for k, v in arglist:
        if k == 'loglevel':
            ...
            thread_count = util_misc.sanitize_thread_count(v)
...
def main_argparse(commands, description):
    ...
    if hasattr(args, 'tmp_dir'):
        ...
        try:
            ...
            if (hasattr(args, 'tmp_dirKeep') and args.tmp_dirKeep) or util_file.keep_tmp():
