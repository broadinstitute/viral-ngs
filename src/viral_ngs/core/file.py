...
def get_test_path():
    '''Return absolute path of "tests" directory'''
...
def check_paths(read=(), write=(), read_and_write=()):
    ...
    '''
    read, write, read_and_write = map(util_misc.make_seq, (read, write, read_and_write))
...
def set_tmp_dir(name):
    ...
    for e in ('LSB_JOBID', 'LSB_JOBINDEX', 'JOB_ID'):
        if e in os.environ:
            ...
            break
    tempfile.tempdir = tempfile.mkdtemp(prefix='-'.join(proposed_prefix) + '-', dir=util_cmd.find_tmp_dir())
...
def extract_tarball(tarfile, out_dir=None, threads=None, compression='auto', pipe_hint=None):
    ...
    if compression == 'zip':
        ...
        if compression == 'gz':
            decompressor = ['pigz', '-dc', '-p', str(util_misc.sanitize_thread_count(threads))]
        elif compression == 'bz2':
            decompressor = ['lbzip2', '-dc', '-n', str(util_misc.sanitize_thread_count(threads))]
...
def concat(inputFilePaths, outputFilePath, append=False):
    ...
    with open(outputFilePath, 'a' if append else 'w') as outfile:
        for filePath in util_misc.make_seq(inputFilePaths):
...
def grep_count(file_path, to_match, additional_flags=None, fixed_mode=True, starts_with=False):
    ...
    number_of_seqs = subprocess.check_output(cmd, env=env)
...
def count_occurrences_in_tsv_sqlite_backed(db_file_path, 
    ...
    with open(file_path) as infile:
        for chunk in util_misc.batch_iterator(csv.reader(infile, delimiter=delimiter), 40000):
...
class CountDB(DBConnection):
    ...
    def increment_count_for_multiple_IDs(self, idvals, increment_val=1):
        with self.conn:
            for idvals_chunk in util_misc.batch_iterator(idvals, 600):
