...
def available_cpu_count():
    ...
    try:
        ...
        if os.path.exists("/sys/fs/cgroup/cgroup.controllers"):
            ...
            cpu_max = util_file.slurp_file("/sys/fs/cgroup/cpu.max").split()
            if cpu_max[0] != "max":
                if len(cpu_max) == 1:
        ...
        # cgroup v1
            ...
            cfs_quota_us = int(util_file.slurp_file("/sys/fs/cgroup/cpu/cpu.cfs_quota_us"))
            cfs_period_us = int(util_file.slurp_file("/sys/fs/cgroup/cpu/cpu.cfs_period_us"))
    ...
    proc_cpus = MAX_INT32
    try:
        m = re.search(r'(?m)^Cpus_allowed:\s*(.*)$', util_file.slurp_file('/proc/self/status'))
        if m:
