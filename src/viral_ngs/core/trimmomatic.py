...
class TrimmomaticTool(Tool):
    ...
    def execute(self,
        ...
        unpairedFastq1 = unpairedOutFastq1 or util_file.mkstempfname()
        unpairedFastq2 = unpairedOutFastq2 or util_file.mkstempfname()
        javaCmd = [trimmomaticPath]

        # Build thread arguments if specified
        thread_args = []
        if threads is not None:
            thread_count = util_misc.sanitize_thread_count(threads)
        ...
        util_misc.run_and_print(javaCmd, check=True)
