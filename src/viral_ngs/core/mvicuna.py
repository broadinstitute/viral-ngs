...
class MvicunaTool(Tool):
    ...
    def rmdup(self, inPair, outPair, outUnpaired=None):
        ...
        """
        if not outUnpaired:
            outUnpaired = util_file.mkstempfname(suffix='.unpaired.fastq')
        tmp1OutPair = (
            util_file.mkstempfname(suffix='.tmp1out.1.fastq'), util_file.mkstempfname(suffix='.tmp1out.2.fastq')
        )
        tmp2OutPair = (
            util_file.mkstempfname(suffix='.tmp2out.1.fastq'), util_file.mkstempfname(suffix='.tmp2out.2.fastq')
        )
        cmdline = [
