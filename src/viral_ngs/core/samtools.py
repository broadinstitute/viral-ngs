...
class SamtoolsTool(Tool):
    ...
    def bam2fq_tmp(self, inBam):
        with util_file.tempfnames(('.1.fq', '.2.fq')) as (reads1, reads2):
            self.bam2fq(inBam, reads1, reads2)
            yield reads1, reads2

    @contextlib.contextmanager
    def bam2fa_tmp(self, inBam):
        with util_file.tempfnames(('.1.fa', '.2.fa')) as (reads1, reads2):
    ...
    def sort(self, inFile, outFile, args=None, threads=None):
        ...
        args = args or []
        if '-@' not in args:
            args.extend(('-@', str(util_misc.sanitize_thread_count(threads))))
    ...
    def faidx(self, inFasta, overwrite=False):
        ...
        if os.path.isfile(outfname):
            if overwrite:
                ...
                return
        #pysam.faidx(inFasta)
        with util_file.fastas_with_sanitized_ids(inFasta, use_tmp=True) as sanitized_fastas:
    ...
    def getHeader(self, inBam):
        ...
        tmpf = util_file.mkstempfname('.txt')
