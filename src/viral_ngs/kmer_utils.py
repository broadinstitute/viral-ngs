...
from .classify import kmc

log = logging.getLogger(__name__)  # pylint: disable=invalid-name

# =================================

def build_kmer_db(seq_files, kmer_db, kmer_size=kmc.DEFAULT_KMER_SIZE, min_occs=1, max_occs=misc.MAX_INT32,
                  counter_cap=kmc.DEFAULT_COUNTER_CAP, single_strand=False, mem_limit_gb=8, mem_limit_laxness=0,
                  threads=None):
    """Build a database of kmers occurring in given sequences."""
    kmc.KmcTool().build_kmer_db(**locals())
...
def parser_build_kmer_db(parser=argparse.ArgumentParser()):
    ...
    cmd.attach_main(parser, build_kmer_db, split_args=True)
    return parser
...
def dump_kmer_counts(kmer_db, out_kmers, min_occs=1, max_occs=misc.MAX_INT32, threads=None):
    """Dump kmers and their counts from kmer database to a text file"""
    kmc.KmcTool().dump_kmer_counts(**locals())
...
def parser_dump_kmer_counts(parser=argparse.ArgumentParser()):
    ...
    cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    cmd.attach_main(parser, dump_kmer_counts, split_args=True)
    return parser
...
def filter_reads(kmer_db, in_reads, out_reads, db_min_occs=1, db_max_occs=misc.MAX_INT32,
    ...
    """
    kmc.KmcTool().filter_reads(**locals())
...
def parser_filter_reads(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('--dbMaxOccs', dest='db_max_occs', type=int, default=misc.MAX_INT32,
    ...
    parser.add_argument('--readMaxOccs', dest='read_max_occs', type=int, default=misc.MAX_INT32,
    ...
    cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    cmd.attach_main(parser, filter_reads, split_args=True)
    return parser
...
def kmers_binary_op(op, kmer_db1, kmer_db2, kmer_db_out,
                    ...
                    result_counter_cap=kmc.DEFAULT_COUNTER_CAP, threads=None):  # pylint: disable=invalid-name
    """Perform a simple binary operation on kmer sets."""

    kmc.KmcTool().kmers_binary_op(**locals())
...
def parser_kmers_binary_op(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('--resultMaxOccs', dest='result_max_occs', type=int, default=misc.MAX_INT32,
                        help='from the result drop kmers with counts above this')
    parser.add_argument('--resultCounterCap', dest='result_counter_cap', type=int, default=kmc.DEFAULT_COUNTER_CAP,
                        help='cap output counters at this value')
    cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    cmd.attach_main(parser, kmers_binary_op, split_args=True)
    return parser
...
def kmers_set_counts(kmer_db_in, value, kmer_db_out, threads=None):
    ...
    kmc.KmcTool().set_kmer_counts(**locals())
...
def parser_kmers_set_counts(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument('kmer_db_out', help='output kmer db')
    cmd.common_args(parser, (('threads', None), ('loglevel', None), ('version', None), ('tmp_dir', None)))
    cmd.attach_main(parser, kmers_set_counts, split_args=True)
    return parser
