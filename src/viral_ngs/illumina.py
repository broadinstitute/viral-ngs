...
from .core.illumina_utils import IlluminaDirectory, RunInfo, SampleSheet, SampleSheetError
...
def parser_illumina_metadata(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, illumina_metadata, split_args=True)
    return parser
...
def splitcode_demux_fastqs(
    ...
    threads = threads or util_misc.available_cpu_count()
    ...
    if has_2bc_samples and not has_3bc_samples:
        ...
        # Use splitcode.create_splitcode_lookup_table() to parse splitcode JSON summary and create CSV
        splitcode_csv_path = os.path.join(outdir, 'splitcode_metrics.csv')
        try:
            splitcode.create_splitcode_lookup_table(
            ...
            picard_style_metrics_path = os.path.join(outdir, 'demux_metrics_picard-style.txt')
            splitcode.convert_splitcode_demux_metrics_to_picard_style(
                splitcode_csv_path,
...
def parser_illumina_demux(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        ...
        nargs='*',
        action=util_cmd.storeMultiArgsOrFallBackToConst,
        type=str,
        const=["barcode_2"],
    )
    parser.add_argument(
    ...
    for opt in picard.ExtractIlluminaBarcodesTool.option_list:
        if opt not in ("read_structure", "num_processors"):
            parser.add_argument(
                ...
                + opt.upper()
                + " (default: %(default)s)",
                default=picard.ExtractIlluminaBarcodesTool.defaults.get(opt),
            )
    for opt in picard.IlluminaBasecallsToSamTool.option_list:
        if opt == "adapters_to_check":
            parser.add_argument(
                ...
                + opt.upper()
                + " (default: %(default)s)",
                default=picard.IlluminaBasecallsToSamTool.defaults.get(opt),
            )
    ...
    parser.add_argument(
        ...
        default=picard.IlluminaBasecallsToSamTool.jvmMemDefault,
    )
    util_cmd.common_args(
        parser,
        (
            (
                "threads",
                picard.IlluminaBasecallsToSamTool.defaults["num_processors"],
            ),
            ("loglevel", None),
            ("version", None),
            ("tmp_dir", None),
    ...
    util_cmd.attach_main(parser, main_illumina_demux)
    return parser
...
def main_illumina_demux(args):
    ...
    try:
        picard.CheckIlluminaDirectoryTool().execute(
        ...
        if os.path.exists(os.path.join(illumina.get_intensities_dir(), "s.locs")):
            ...
            broken_links = util_file.find_broken_symlinks(
    ...
    if multiplexed_samples:
        ...
        if args.commonBarcodes:
            ...
            util_file.touch(args.commonBarcodes)
    ...
    if multiplexed_samples:
        ...
        extract_input = util_file.mkstempfname(
        ...
        out_metrics = (
            ...
            and util_file.mkstempfname(".metrics.txt")
            or args.outMetrics
        )
        picardOpts = dict(
        ...
        picard.ExtractIlluminaBarcodesTool().execute(
            illumina.get_BCLdir(),
            args.lane,
            extract_input,
            barcodes_tmpdir,
        ...
        if args.commonBarcodes:
            ...
            async_execution_futures.append(
                executor.submit(
                    ...
                    threads=util_misc.sanitize_thread_count(args.threads),
                )
            )

        # Picard IlluminaBasecallsToSam
        basecalls_input = util_file.mkstempfname(
    ...
    if picardOpts.get("sequencing_center"):
        picardOpts["sequencing_center"] = util_file.string_to_file_name(
    ...
    gc.collect()
    if multiplexed_samples:
        picard.IlluminaBasecallsToSamTool().execute(
            illumina.get_BCLdir(),
            barcodes_tmpdir,
            flowcell,
            args.lane,
        ...
        picard.IlluminaBasecallsToSamTool().execute_single_sample(
            illumina.get_BCLdir(),
...
def parser_flowcell_metadata(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, main_flowcell_metadata)
    return parser
...
def parser_lane_metrics(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        "--JVMmemory",
        help="JVM virtual memory size (default: %(default)s)",
        default=picard.ExtractIlluminaBarcodesTool.jvmMemDefault,
    )
    util_cmd.common_args(
        parser, (("loglevel", None), ("version", None), ("tmp_dir", None))
    )
    util_cmd.attach_main(parser, main_lane_metrics)
    return parser
...
def main_lane_metrics(args):
    ...
    picardOpts = dict(
        (opt, getattr(args, opt))
        for opt in picard.CollectIlluminaLaneMetricsTool.option_list
        if hasattr(args, opt) and getattr(args, opt) != None
    )
    picardOpts["read_structure"] = read_structure
    picard.CollectIlluminaLaneMetricsTool().execute(
        illumina.path,
        output_dir,
        output_prefix,
        picardOptions=picardOpts,
...
def parser_common_barcodes(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, main_common_barcodes)
    return parser
...
def main_common_barcodes(args):
    ...
    # Picard ExtractIlluminaBarcodes
    barcode_file = util_file.mkstempfname(
        ".txt", prefix=".".join(["barcodeData", flowcell, str(args.lane)])
    )
    barcodes_tmpdir = tempfile.mkdtemp(prefix="extracted_barcodes-")
...
def count_and_sort_barcodes(
    ...
    with util_file.CountDB() as reduce_db:
        barcodefile_tempfile_tuples = [
            (tile_barcode_file, util_file.mkstempfname("sqlite_.db"))
        ...
        workers = util_misc.sanitize_thread_count(threads)
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            for bf, tf in barcodefile_tempfile_tuples:
                ...
                futures.append(
                    executor.submit(
                        util_file.count_occurrences_in_tsv_sqlite_backed,
                        tf,
                        bf,
...
def parser_guess_barcodes(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, main_guess_barcodes, split_args=True)
    return parser
...
# NOTE: IlluminaDirectory, RunInfo, SampleSheet, SampleSheetError classes
# have been moved to core.illumina_utils
...
def miseq_fastq_to_bam(
    ...
    runInfo=None,
    sequencing_center=None,
    JVMmemory=picard.FastqToSamTool.jvmMemDefault,
):
...
def parser_miseq_fastq_to_bam(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        "--JVMmemory",
        default=picard.FastqToSamTool.jvmMemDefault,
        help="JVM virtual memory size (default: %(default)s)",
    )
    util_cmd.common_args(
        parser, (("loglevel", None), ("version", None), ("tmp_dir", None))
    )
    util_cmd.attach_main(parser, miseq_fastq_to_bam, split_args=True)
    return parser
...
def parser_extract_fc_metadata(parser=argparse.ArgumentParser()):
    ...
    util_cmd.attach_main(parser, extract_fc_metadata, split_args=True)
    return parser
...
def run_picard_fastq_to_ubam(fq1,
    ...
    if string_to_log:
        log.info(string_to_log)
    picard.FastqToSamTool().execute(  fq1,
                                            fq2,
                                            sample_name,
                                            out_bam,
...
def splitcode_demux(
    ...
    sequencing_center = util_file.string_to_file_name(sequencing_center)
    platform_model    = util_file.string_to_file_name(platform_model)
    ...
    #       
    # 
    #with samtools.SamtoolsTool().bam2fq_tmp(inBam) as (fqin1, fqin2), \
    #     util_file.tmp_dir('_splitcode') as t_dir:
    #     pass
    ...
    for pool_id, sample_libraries in pool_id_to_sample_library_id_map.items():
        ...
        splitcode_config, splitcode_sample_keep_file, returned_sample_ids = splitcode.generate_splitcode_config_and_keep_files(
            inner_demux_barcode_map_df,
            pool_id,
    ...
    # ======== run splitcode ==========
    workers = threads or util_misc.available_cpu_count()
    ...
    if threads:
        ...
        threads_per_worker = util_misc.available_cpu_count() // workers
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for pool_idx,pool_id in enumerate(sorted(pool_id_to_sample_library_id_map.keys())):
            ...
            future = executor.submit(
                                        splitcode.run_splitcode_on_pool,
                                        pool_id,
                                        pool_bam_file,
                                        splitcode_config,
                                        splitcode_keepfile,
    ...
    # (avoiding mismatch between SampleSheet.inner_demux_mapper() and splitcode.create_splitcode_lookup_table())
    df_csv_out_path = splitcode.create_splitcode_lookup_table(
    ...
    splitcode.convert_splitcode_demux_metrics_to_picard_style(df_csv_out_path, picard_style_splitcode_metrics_path)
    ...
    splitcode.plot_read_counts(df_csv_out_path, outDir)
    # Plot a sorted curve per pool and save csv file with sorted read numbers and fractions for QC
    splitcode.plot_sorted_curve(df_csv_out_path, outDir, unmatched_name)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for sample_idx,(sample,(fq1,fq2)) in enumerate(sample_library_id_to_fastqs.items()):
            ...
            readgroup_name = ".".join([
                                        flowcell[:4],
                                        str(lane),
                                        str(samplesheet_row_idx+1) if samplesheet_row_idx is not None else util_misc.md5_digest(barcodes_for_sample_joined,last_n_chr=4)
                                        ])
            picardOptions = [
...
def parser_splitcode_demux(parser=None):
    ...
    util_cmd.attach_main(parser, main_splitcode_demux)
    return parser
...
def parser_merge_demux_metrics(parser=argparse.ArgumentParser()):
    ...
    util_cmd.common_args(parser, (('loglevel', None), ('version', None), ('tmp_dir', None)))
    util_cmd.attach_main(parser, merge_demux_metrics, split_args=True)
    return parser
