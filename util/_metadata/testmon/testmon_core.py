...
class Testmon(object):
    """Class that provides the top-level API to pytest-testmon.  Orchestrates the collection of coverage data and the identificationn
    of tests that need re-running.  The class pytest_testmon.TestmonDeselect stores an instance of this class."""
...
class TestmonData(object):
    """Manages the storage of test outcome data.  Implemented using a SQLite database.

    Fields:
        variant: a string identifying the platform variant (pyton version, os version etc).
        rootdir: the root directory of the source code hierarchy
        datafile: path to the testmon data file
        connection: connection object for the SQLite database
        node_data: map from nodeid of a test, to map from filename of a python file (containing code that may affect the test)
            to list of checksums of blocks in that file.
        reports: test reports for all tests that have been run
        fail_reports: test reports for tests that have failed
    """
...
""")
    ...
    def get_nodedata(self, nodeid, coverage_data, rootdir):
        """Given coverage data, return map from filename to checksums of the covered parts of that file."""
        result = {}
    ...
    def set_dependencies(self, nodeid, nodedata, result=[]):
        """Record, for one test, which code parts may affect that test.

        Args:
            nodeid: the pytest node id for the given test
            nodedata: map from filename to a list of checksums 
        """
