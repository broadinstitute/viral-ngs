...
# * Serialization of reports of failed tests

# When re-running a test suite, if a test has failed in the past and none of the
# code used by the test has changed, we don't need to re-run the test, but we
# do still need to print the error messages that would be output by the test if we
# did re-run it.  We therefore store, in TestmonData, the failure report of each test that was run and failed.
# Code below deals with serializingn/unserializing these failed test reports for storage.
...
class TestmonDeselect(object):
    """The main class implementing the testmon plugin: pytest interfaces with the plugin through this class."""
    ...
    def report_if_failed(self, nodeid):
        """If the given test failed the last time we ran it, add the failure report to the current test log."""
    ...
    def pytest_collection_modifyitems(self, session, config, items):
        """Identify which tests can be skipped, and tell pytest."""

        # If some tests have been removed from the codebase, purge information about the results
        # of these tests from our database of test results.
    ...
    def pytest_runtest_logreport(self, report):
        """Save, to our database of test results, the log report for a test that has been run (whether it failed or not)."""
