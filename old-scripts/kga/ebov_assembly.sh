...
for sample in G3789.1 G3823 G3823_r1 G3789.3
do
for directory in /idi/sabeti-data/kandersen/analysis/141017_danny-ebov
do
for suffix in sorted.bam
do
for in_url in /idi/sabeti-data/kandersen/sequencing_storage/ebov/bam_sorted
do
...
#-------- PERFORM DE NOVO ASSEMBLY OF EBOV READS USING TRINITY --------#
...
#-------- MAP EBOV READS TO MODIFIED CONTIGS USING NOVOALIGN --------#
...
for directory in /idi/sabeti-data/kandersen/analysis/141017_danny-ebov
do
bsub -W 4:00 -q hour -R "rusage[mem=2]" -o $directory/_logs/$sample.log.bsub.txt -P sabeti_kga -J $sample.ix "cat $directory/_temp/$sample.s_segment4.fasta > $directory/_refs/$sample.fasta && java -Xmx2g -jar /seq/software/picard/current/bin/CreateSequenceDictionary.jar R=$directory/_refs/$sample.fasta O=$directory/_refs/$sample.dict && /idi/sabeti-scratch/kandersen/bin/novocraft/novoindex $directory/_refs/$sample.nix $directory/_refs/$sample.fasta && samtools faidx $directory/_refs/$sample.fasta"
done
done
