...
class TestRunInfo(TestCaseWithTmp):
    ...
    def test_nextseq_1000_2000_p1(self):
        ...
        self.assertEqual(runinfo.get_machine(), 'VH01619')
        self.assertEqual(runinfo.get_read_structure(), '151T8B8B151T')
        self.assertEqual(runinfo.num_reads(), 2)
        self.assertEqual(runinfo.get_machine_model(), "NextSeq 1000/2000")
        self.assertEqual(runinfo.get_flowcell_lane_count(), 1)
...
class TestDifficultSampleNames(TestCaseWithTmp):
    ...
    def test_paired_1(self):
        ...
        outHeader = viral_ngs.core.file.mkstempfname('.txt')
    ...
    def test_inline_commas_strings(self):
        ...
        for sample_name in names_to_validate:
            assert viral_ngs.core.file.string_to_file_name(sample_name) in sample_names


class TestIlluminaBarcodeHelper(TestCaseWithTmp):
    def test_one_correction(self):
        dir_prefix = "one_correction"
        in_dir = viral_ngs.core.file.get_test_input_path(self)
...
class TestSplitcodeDemuxIntegration(TestCaseWithTmp):
    ...
    def setUp(self):
        ...
        self.picard = viral_ngs.core.picard.FastqToSamTool()
    ...
    def _multilib_pool_bam_name(self, samplesheet, run_id):
        """Return the pool BAM basename the OUTER demux would write for a sheet.

        Derived rather than hard-coded: the outer demux names its collapsed
        pool BAM from SampleSheet.collapse_sample_index_duplicates(), which
        folds every differing column through collapse_dup_strs_to_str_or_md5.
        Reproducing that here keeps the fixture honest if the collapse rule
        ever changes, and documents that the two are coupled.
        """
        samples = viral_ngs.illumina.SampleSheet(
            samplesheet, allow_non_unique=True, append_run_id=run_id)
        samples.collapse_sample_index_duplicates()
        pool_rows = [r["run"] for r in samples.get_rows()]
        self.assertEqual(len(pool_rows), 1, f"expected a single collapsed pool, got {pool_rows}")
        return pool_rows[0] + ".bam"
    ...
    def test_splitcode_demux_pool_with_mixed_library_ids(self):
        ...
        with tempfile.TemporaryDirectory() as input_bams_dir:
            with tempfile.TemporaryDirectory() as outDir:
                pool_bam = self._multilib_pool_bam_name(samplesheet, "TESTFLOW.1")
                self.create_test_bam_with_inline_barcodes(
...
class TestIlluminaMetadata(TestCaseWithTmp):
    ...
    def setUp(self):
        ...
        test_dir = viral_ngs.core.file.get_test_path()
...
class TestMergeDemuxMetrics(TestCaseWithTmp):
    ...
    def test_merge_demux_metrics_via_parser(self):
        ...
        out_file = viral_ngs.core.file.mkstempfname('.txt')
