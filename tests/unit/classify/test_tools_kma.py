...
@pytest.fixture
def kma_tool(mocker):
    mocker.patch('kma.shutil.which', return_value='/usr/bin/kma')
    return kma.KMA()
...
def _mock_common_io(mocker, mkstemp_paths, *, is_empty=False):
    mocker.patch('kma.util_file.mkstempfname', side_effect=mkstemp_paths)
    mocker.patch('kma.os.unlink')

    picard_cls = mocker.patch('kma.picard.SamToFastqTool', autospec=True)
    ...
    mocker.patch('kma.picard.PicardTools.dict_to_picard_opts', return_value='clip-opts')

    samtools_cls = mocker.patch('kma.samtools.SamtoolsTool', autospec=True)
    samtools = samtools_cls.return_value
    samtools.isEmpty.return_value = is_empty

    mock_check_call = mocker.patch('kma.subprocess.check_call', autospec=True)
...
def test_classify_single_end_from_bam(mocker, kma_tool, kma_inputs):
    ...
    mocker.patch('kma.os.path.getsize', side_effect=lambda path: size_map.get(path, 1000))
...
def test_classify_returns_early_when_bam_is_empty(mocker, kma_tool, kma_inputs):
    ...
    mock_open = mocker.patch('kma.open', mocker.mock_open())
