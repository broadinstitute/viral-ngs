...
def make_params_file(inFile, bamDir, outFile):
    ...
    with open(outFile, 'wt') as outf:
        ...
        for row in rows:
            ...
            if row.get('library_id_per_sample'):
                out['LIBRARY_NAME'] += '.' + str(row['library_id_per_sample'])
            out['OUTPUT'] = out['LIBRARY_NAME']
            if row.get('run_id_per_library'):
                out['OUTPUT'] += '.r' + str(row['run_id_per_library'])
            out['OUTPUT'] = os.path.join(bamDir, out['OUTPUT'] + ".bam")
            out['ID'] = hashlib.sha1(out['OUTPUT'].encode('utf-8')).hexdigest().upper()[:8]
            outf.write('\t'.join(out[h] for h in header)+'\n')
...
def main_make_params_file(args) :
    make_params_file(args.barcodeFile, args.run, args.bamDir, args.outFile)
    return 0
