...
if [ ! -z "$TRAVIS_ACCESS_TOKEN_FOR_OTHER_REPO" ]; then
    ...
    if [ $TRAVIS_PULL_REQUEST != "false" -o $TRAVIS_BRANCH = "master" -o -n "$TRAVIS_TAG" ]; then
        ...
        if [ -n "$TRAVIS_TAG" ]; then
            ./travis/trigger-travis.sh --script "\"export UPSTREAM_BRANCH=$TRAVIS_BRANCH; export UPSTREAM_TAG=$TRAVIS_TAG; sh travis/tests-long.sh\"" broadinstitute viral-ngs-deploy $TRAVIS_ACCESS_TOKEN_FOR_OTHER_REPO "UPSTREAM_BRANCH=$TRAVIS_BRANCH UPSTREAM_TAG=$TRAVIS_TAG"
        else
            ./travis/trigger-travis.sh --script "\"export UPSTREAM_BRANCH=$TRAVIS_BRANCH; sh travis/tests-long.sh\"" broadinstitute viral-ngs-deploy $TRAVIS_ACCESS_TOKEN_FOR_OTHER_REPO "UPSTREAM_BRANCH=$TRAVIS_BRANCH"
        fi
    fi
else
