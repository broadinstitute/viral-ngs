...
if [ $BUILD_PACKAGE = "true" ]; then
    ...
    if [[ ( -n $TRAVIS_PULL_REQUEST && $TRAVIS_PULL_REQUEST != "false" ) || $TRAVIS_BRANCH = "master" || -n "$TRAVIS_TAG" ]]; then
        ...
        if [ -n "$TRAVIS_TAG" ]; then
            ...
            python packaging/conda-recipe/render-recipe.py "0.0.0+$package_suffix" --package_name "viral-ngs-dev" --download-filename "$TRAVIS_BRANCH" --build-reqs requirements-conda.txt --run-reqs requirements-conda.txt --py3-run-reqs requirements-py3.txt --py2-run-reqs requirements-py2.txt --test-reqs requirements-conda-tests.txt && \
