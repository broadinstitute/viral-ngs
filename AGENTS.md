...
3. For x86-only packages (no ARM64 build), add to the appropriate `-x86.txt` file:
   - `core-x86.txt` - novoalign, cd-hit-auxtools
...
- x86-only packages (novoalign, cd-hit-auxtools, bmtagger, kallisto, table2asn) handled via `--x86-only:` prefix in `install-conda-deps.sh`
